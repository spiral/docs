...
> Spiral Stempler isn't intended to create or replace basic control structures in your templates. If you want to replace PHP as you primary templating language - you can create a custom view processor/modifier and define your own language (for example using Twig, Smarty, Blade, Latte etc).
...
> Attention, stempler does not know about changes in parent layouts (in current version), you should either recompile your views using the `views:compile` command or simply disable the cache in the development enviroment (see [configuring enviroment](/installation.md)).
