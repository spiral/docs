...
##### Sub/Nested Queries
Every spiral QueryBuilder is as instance of `SQLFramegmentInterface` which means you can freely use such sub queries as arugments in where methods. For example let's create some dummy query:
...
QueryBuilders (especially all this weird where statements) can seem useless as we still can write our queiries in plain SQL, hovewer they will simplify your life a lot once you will start working with databases and table prefixes and especially with ORM and pre-loading relations.
