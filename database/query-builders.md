...
```dump
dump($db->users->select()->columns('name')->queryString());
```

> Attention, queryString method returns interpolated query (will inserted where values), do not use it run a query. 
> You can access SQL with value placeholders using `sqlStatement` method.
...
```php
$select->where(
    $db->select('COUNT(*)')->from('test')->where('id', 'BETWEEN', 10, 100), '>', 1
);
```
