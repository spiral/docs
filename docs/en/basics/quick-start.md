...
The application dependencies are defined in `composer.json` and activated in `app/src/App.php` as Bootloaders.
...
Delete the following bootloaders from `app/src/App.php`:
...
By default, the routing rules are located in `app/src/Bootloader/RoutesBootloader.php`. You have many options on how
