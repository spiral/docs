...
Since this is only one global instance you are still able (theorecially) to isolate multiple applications in one shared memory by swapping global container before calling application enterpoint (attention, `staticContainer()` method is protected for `Component` members only):
