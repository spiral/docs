# Controllers
Controllers as it stated in their name are responsinble for controlling application flow, they usually access user request, differenrt models (services) and provides content to be sent to client using JSON arrays or view component. Basic controller example may look like (default controller action is 'index'):
...
> Output generated by controllers usually send directly to MiddlewarePipeline which will wrap it into valid response, meaning you can return strings and arrays from
your controllers.
